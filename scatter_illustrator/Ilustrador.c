...
typedef enum {INT, DOUBLE, PONTEIRO} tTipoVariavel;
...
int IlustraCriaVariavelInteira(const int *v, const char *nome){
    ...
    sprintf(novaAcao.comando, "NOVO|VAR|%d|%s|INT|%d\n", novoObjeto.id, novoObjeto.nome, *v);
...
int IlustraCriaArrayDeInteiros(int ar[], int t, const char *nome){
    //NOVO|ARRAY_INT|id|NOME|t|elem0|elem1|..|elemN
    ...
    novaAcao.tipoAcao = NOVO;
    novaAcao.indiceObjeto = qObjetos-1;
    sprintf(novaAcao.comando, "NOVO|ARRAY_INT|%d|%s|%d%s\n", novoObjeto.id, novoObjeto.nome, t, elementos);
    acoes[qAcoes++] = novaAcao;
